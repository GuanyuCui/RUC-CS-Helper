...
bool isNextInPar(char * now, const char * strHead)
{
    ...
    //找到之前的第一个非空格字符
    while(pCursor > strHead && (*pCursor == ' ' || *pCursor == '\t'))
    {
        pCursor--;
    }
    if(*pCursor == '(')
...
void Calculator<T>::printLogo()
{
    ...
    std::cout << "|                                    |" << std::endl;
    std::cout << "|    Written by G.Cui, 2019.10.19    |" << std::endl;
    std::cout << "--------------------------------------" << std::endl;
}
...
//—————————————————————————栈———————————————————————————
template <typename T>
double Calculator<T>::calcExpr(std::string & expr)
{
    //操作数栈
